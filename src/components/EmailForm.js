"use client";

import { useForm } from "react-hook-form";
import { sendEmail } from "@/utils/send-email";

const EmailForm = () => {
  const { register, handleSubmit } = useForm();

  function onSubmit(data) {
    // Call the sendEmail function with the form data
    // This function should be defined in your utils/send-email.js file

    sendEmail(data);
  }

  return (
    <form onSubmit={handleSubmit(onSubmit)} id="contactForm">
      <div className="mb-5">
        <label htmlFor="name" className="mb-3 block text-base font-medium">
          Full Name
        </label>
        <input
          type="text"
          placeholder="Full Name"
          className="w-full rounded-md border border-gray-300 bg-white py-3 px-6 text-base font-medium text-gray-700 outline-none focus:border-cyan-400 focus:shadow-md"
          {...register("name", { required: true })}
        />
      </div>
      <div className="mb-5">
        <label htmlFor="email" className="mb-3 block text-base font-medium">
          Email Address
        </label>
        <input
          type="email"
          placeholder="example@domain.com"
          className="w-full rounded-md border border-gray-300 bg-white py-3 px-6 text-base font-medium text-gray-700 outline-none focus:border-cyan-400 focus:shadow-md"
          {...register("email", { required: true })}
        />
      </div>
      <div className="mb-5">
        <label htmlFor="message" className="mb-3 block text-base font-medium">
          Message
        </label>
        <textarea
          rows={4}
          placeholder="Type your message"
          className="w-full resize-none rounded-md border border-gray-300 bg-white py-3 px-6 text-base font-medium text-gray-700 outline-none focus:border-cyan-400 focus:shadow-md"
          {...register("message", { required: true })}
        ></textarea>
      </div>
      <div>
        <button
          id="submitForm"
          className="hover:shadow-form rounded-md bg-cyan-400 py-3 px-8 text-base font-semibold text-white outline-none transition-all duration-700"
        >
          Submit
        </button>
      </div>
    </form>
  );
};

export default EmailForm;
